...
export default function VerifyPage() {
	...
	useEffect(() => {
		...
		apiGet(`/auth/verify?token=${encodeURIComponent(token)}&email=${encodeURIComponent(email)}`)
			.then(() => setState("ok"))
			.catch((e: any) => {
				setState("fail");
	...
	return (
		<div className="mx-auto max-w-md px-4 sm:px-6 lg:px-8 py-10">
			<h1 className="text-3xl sm:text-4xl font-bold text-center">Verify email</h1>

			{state === "loading" && <p className="mt-6 text-center">Verifying…</p>}
			...
			{state === "fail" && (
				<div className="mt-6 rounded-md border border-rose-500/30 bg-rose-50 dark:bg-rose-900/10 p-4">
					...
					<p className="opacity-80">
						{reason ? `Reason: ${reason}` : "Please request a new verification email."}
					</p>
				</div>
			)}
		</div>
