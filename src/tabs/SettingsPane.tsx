...
function Row({
	...
	return (
		<div className="flex items-start justify-between gap-4 py-3">
			<div>
				{htmlFor ? (
					<label htmlFor={htmlFor} className="font-medium leading-none cursor-pointer">
						{label}
					</label>
				) : (
				...
				{description ? (
					<div className="text-sm text-neutral-600 dark:text-neutral-400 mt-1 max-w-prose">{description}</div>
				) : null}
			</div>
...
function SmallSwitch({
	...
	return (
		<label className="inline-flex items-center cursor-pointer select-none" title={title || ariaLabel}>
			<input
				id={id}
				type="checkbox"
...
function SettingsCore() {
			...
			try {
				...
				if (settingsFetchedFromServer && cachedSettings) {
					...
					next = {
						...
						showTimestamps: s.showTimestamps === undefined ? true : !!s.showTimestamps,
						compactMode: s.compactMode === undefined ? false : !!s.compactMode,
					};
	...
	}, [loadedFromServer, settings.saveNewChatsToCloud, settings.theme, settings.showTimestamps, settings.compactMode]);

	// ---------- Broadcast to other tabs (ChatPane) ----------
	useEffect(() => {
		window.dispatchEvent(new CustomEvent("ysong:settings", { detail: settings }));
	}, [settings]);
	...
	return (
		<div className="h-full flex flex-col">
			<div className="flex-1 min-h-0 overflow-y-auto" style={{ scrollbarGutter: "stable both-edges" } as any}>
				<div className="p-4 md:p-6 lg:p-8 mx-auto max-w-3xl space-y-6 pb-16">
					<header className="flex items-center gap-3">
						<div className="rounded-2xl bg-indigo-500/10 p-2" aria-hidden>
							⚙️
						</div>
						<div>
							<h1 className="text-2xl font-semibold tracking-tight">Settings</h1>
							<p className="text-neutral-600 dark:text-neutral-400">
								Tune YSong to your liking. Changes save automatically.
							</p>
						</div>
					</header>
									...
									<SmallSwitch
										...
										onChange={(v) => {
											...
											saveUserSettings({
												...
												console.error("saveUserSettings(saveChats) failed", err);
											});
										}}
									/>
								}
									...
									<SmallSwitch
										...
										onChange={(v) => {
											...
											saveUserSettings({
												...
												console.error("saveUserSettings(showTimestamps) failed", err);
											});
										}}
									/>
								}
									...
									<SmallSwitch
										...
										onChange={(v) => {
											...
											saveUserSettings({
												...
												console.error("saveUserSettings(compactMode) failed", err);
											});
										}}
									/>
								}
	...
	return (
		<YSButton
			className={`text-sm underline underline-offset-2 ${
				ok ? "text-green-600 dark:text-green-400" : "text-indigo-600 dark:text-indigo-300"
			}`}
			onClick={async () => {
				try {
...
function DarkModeRow({ dark, onToggle }: { dark: boolean; onToggle: (v: boolean) => void }) {
	return (
		<Row
			label="Dark mode"
