  ...
  return <div className="h-full min-h-0 overflow-y-auto bg-neutral-50 dark:bg-neutral-950 text-neutral-900 dark:text-neutral-100">
    <div className="mx-auto max-w-6xl px-5 py-7 md:px-8 md:py-9">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="text-right text-xs"><div className={health === "online" ? "text-emerald-500" : health === "offline" ? "text-rose-500" : "text-neutral-500"}>{health === "online" ? "Audio Engine online" : health === "offline" ? "Audio Engine offline" : "Checking engine…"}</div><div className={capabilities?.available ? "mt-1 text-emerald-500" : "mt-1 text-amber-500"}>{capabilities?.available ? "YSong Ears ML backend ready" : capabilities ? "YSong Ears ML backend unavailable" : "Checking CLAP…"}</div></div>
      </div>
      ...
      {report && upload && <>
        ...
        <div className="mt-5 rounded-2xl border border-dashed border-neutral-300 dark:border-neutral-800 p-4 text-xs text-neutral-500">Analysis policy: learned labels are candidates, not ground truth. Explicit status is never guessed without lyrics. AI-likelihood is deliberately capped and cannot establish provenance. The canonical saved metadata can be reused without re-listening.</div>
      </>}
    </div>
  </div>;
}
