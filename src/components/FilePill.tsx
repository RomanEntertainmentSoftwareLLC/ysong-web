...
import { audioController, useAudioControllerState, type AudioAsset } from "./AudioController";
...
function drawStereoWaveform(canvas: HTMLCanvasElement, peaks: WaveformPeaks, hue: number) {
	...
	const drawChannel = (arr: Float32Array, midY: number, stroke: CanvasGradient) => {
...
async function fetchSignedUrl(objectKey: string, mode: "play" | "download") {
	...
	const url = `${API_BASE}/api/uploads/signed-url?objectKey=${encodeURIComponent(objectKey)}&mode=${mode}`;
...
export function FilePill({ id, name, sizeMB, type, publicUrl, objectKey, onDelete, onDownload, disableScrub, onAddToProject, className, style }: FilePillProps) {
	...
	useEffect(() => {
		...
		const run = async () => {
			try {
				...
				if (objectKey) {
					...
					const needsSigned = looksLikeGcsUrl(publicUrl) && !isLikelySignedUrl(publicUrl);
					if (needsSigned) {
						const key = deriveObjectKeyFromPublicUrl(publicUrl);
						url = key ? (await fetchSignedUrl(key, "play")).url : publicUrl;
					} else {
						url = publicUrl;
					}
				}
		...
		const cancelIdle = (window as any).cancelIdleCallback as ((id: any) => void) | undefined;

		const idleId = idle ? idle(run, { timeout: 1200 }) : window.setTimeout(run, 250);
	...
	const onPlayStop = async () => {
		...
		if (
			isCurrent &&
			(audioState.status === "playing" || audioState.status === "paused" || audioState.status === "loading")
		) {
	...
	useLayoutEffect(() => {
		...
		const update = () => {
			...
			const top = fitsBelow ? r.bottom + gap : Math.max(8, r.top - gap - menuH);

			const leftIdeal = r.right - menuW;
			const left = Math.min(Math.max(8, leftIdeal), window.innerWidth - menuW - 8);
	...
	useLayoutEffect(() => {
		...
		const update = () => {
			...
			const placement: "above" | "below" = spaceAbove >= 56 || spaceAbove >= spaceBelow ? "above" : "below";
	...
	return (
		...
		>
			...
			>
				...
				{isAudio && (
					<div className={`fp-wave ${waveform ? "is-real" : ""}`} aria-hidden="true">
						{waveform ? (
							<canvas ref={waveCanvasRef} className="fp-wave-canvas" />
						) : (
							Array.from({ length: 52 }).map((_, i) => {
								const h = 20 + ((i * 17) % 60);
								return <span key={i} className="fp-wave-bar" style={{ height: `${h}%` }} />;
							})
						)}
					</div>
				)}
				...
				{isAudio && (playState !== "stopped" || playheadPct > 0) && (
					<div className="fp-playhead" style={{ left: `${playheadPct}%` }} aria-hidden="true" />
				)}
					...
					<div className="fp-top">
						...
						{isAudio && (
							<div
								...
								title={
									durationKnown
										? `${fmtTime(shownCurrent)} / ${fmtTime(shownDuration)}`
				...
				{isAudio && (
					<div className="fp-controls" aria-label="Audio controls">
						<button
							...
							onClick={onPlayStop}
							disabled={!canPlay}
							aria-label={playState === "stopped" ? "Play" : "Stop"}
							title={
								!canPlay
									? objectKey
										? "Missing audio URL"
						...
						<button
							...
							title={!canPlay ? "No audio URL" : loopOn ? "Loop: On" : "Loop: Off"}
						>
				...
				createPortal(
					...
					>
						<button
							...
							title={objectKey || publicUrl ? "Download" : "No URL available"}
						>
							Download
						</button>
