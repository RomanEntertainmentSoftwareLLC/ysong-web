						...
						{chats.map((c) => {
							...
							return (
								<div key={c.id} className="group relative">
									{/* main row button */}
									<YSButton
										onClick={() => (onOpenChatTab ? onOpenChatTab(c) : setActiveId(c.id))}
			...
			{menuFor &&
				createPortal(
					<>
						...
						<div className="fixed inset-0 z-[100] bg-transparent" onClick={() => setMenuFor(null)} />
						<div
						...
						>
                           ...
                           bg-white dark:bg-neutral-900 shadow-xl p-1"
							>
								<MenuItem
									onClick={() => {
										const current = chats.find((x) => x.id === menuFor);
										const name = window.prompt(
											"Rename chat",
											current ? chatLabel(current) : "Untitled"
										);
										if (name && name.trim()) onRenameChat?.(menuFor, name.trim());
										setMenuFor(null);
									}}
								>
									{/* pencil icon */}
