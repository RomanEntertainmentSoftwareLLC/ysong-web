 ...
 */
export default function UINavbar({ meEmail, onLogout, onToggleSidebar }: Props) {
	const { dark } = useTheme();
	...
	useEffect(() => {
		if (menuBtnRef.current) {
			menuBtnRef.current.setAttribute("aria-expanded", menuOpen ? "true" : "false");
		}
	}, [menuOpen]);
					...
					{meEmail && onLogout && (
						<div className="relative">
							<YSButton
								...
								title={meEmail}
							>
								<span className="hidden sm:inline max-w-[14ch] truncate">{meEmail}</span>
							...
							{menuOpen && (
								...
								>
									...
									<div className="px-2 py-1 text-xs opacity-70 truncate">{meEmail}</div>
