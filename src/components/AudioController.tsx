...
export type AudioTargetRef = Partial<Pick<AudioAsset, "id" | "objectKey" | "publicUrl" | "name">>;
...
async function fetchSignedUrl(objectKey: string, purpose: SignedUrlPurpose): Promise<string> {
	const API = (import.meta as any).env?.VITE_AUTH_API_URL || "";
	const endpoint = API ? `${API}/api/uploads/signed-url` : `/api/uploads/signed-url`;

	const token = localStorage.getItem("ys_token");
	if (!token) throw new Error("missing_token");

	const url = `${endpoint}?objectKey=${encodeURIComponent(objectKey)}&mode=${encodeURIComponent(purpose)}`;
	...
	if (!res.ok) {
		const msg = data?.error ? String(data.error) : `signed_url_failed_${res.status}`;
		throw new Error(msg);
	}
...
export class AudioController {
	...
	registerAssets(list: AudioAsset[]) {
		for (const a of list) {
			...
			if (merged.objectKey) this.byObjectKey.set(merged.objectKey, merged.id);
			if (merged.publicUrl) this.byPublicUrl.set(merged.publicUrl, merged.id);
		}
	}
	...
	private resolve(ref: AudioTargetRef): AudioAsset | undefined {
		...
		if (objectKey) {
			...
			if (mapped && this.assets.has(mapped)) return this.assets.get(mapped);
		}
		if (publicUrl) {
...
export function useAudioControllerState() {
	return useSyncExternalStore(audioController.subscribe, audioController.getSnapshot, audioController.getSnapshot);
}
